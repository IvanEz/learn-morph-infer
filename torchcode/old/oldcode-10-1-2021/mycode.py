...
class BasicBlockInv_Pool_constant_noBN_n4_inplace_leaky(torch.nn.Module):
    def __init__(self, inplanes, downsample=False):
        super(BasicBlockInv_Pool_constant_noBN_n4_inplace_leaky, self).__init__()
        ...
        #self.bn1 = torch.nn.BatchNorm3d(inplanes)
        self.conv1 = conv3x3_biased(inplanes, inplanes)
        self.relu1 = torch.nn.LeakyReLU(negative_slope=0.3, inplace=True)

        #self.bn2 = torch.nn.BatchNorm3d(inplanes)
        self.conv2 = conv3x3_biased(inplanes, inplanes)
        self.relu2 = torch.nn.LeakyReLU(negative_slope=0.3, inplace=True)

        self.conv3 = conv3x3_biased(inplanes, inplanes)
        self.relu3 = torch.nn.LeakyReLU(negative_slope=0.3, inplace=True)

        self.conv4 = conv3x3_biased(inplanes, inplanes)
        self.relu4 = torch.nn.LeakyReLU(negative_slope=0.3, inplace=True)
...
class inorm_block_convnormrelu(torch.nn.Module): #does not begin with instancenorming, affine instancenorm
    def __init__(self, inplanes, normalize=False):
        super(inorm_block_convnormrelu, self).__init__()
        self.normalize = normalize
        #self.downsample = downsample
        #if self.downsample:
        #    self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
...
class inorm_block4(torch.nn.Module): #does not begin with instancenorming, affine instancenorm
    def __init__(self, inplanes, normalize=False):
        super(inorm_block4, self).__init__()
        self.normalize = normalize
        #self.downsample = downsample
        #if self.downsample:
        #    self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        ...
        if self.normalize:
            ...
            self.norm1 = torch.nn.InstanceNorm3d(inplanes,affine=True)
            self.norm2 = torch.nn.InstanceNorm3d(inplanes,affine=True)
            self.norm3 = torch.nn.InstanceNorm3d(inplanes,affine=True)
            self.norm4 = torch.nn.InstanceNorm3d(inplanes,affine=True)
...
class inorm_block_convnormrelu_4(torch.nn.Module): #does not begin with instancenorming, affine instancenorm
    def __init__(self, inplanes, normalize=False):
        super(inorm_block_convnormrelu_4, self).__init__()
        self.normalize = normalize
        #self.downsample = downsample
        #if self.downsample:
        #    self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
...
class inorm_block_different(torch.nn.Module): #does not begin with instancenorming, affine instancenorm
    def __init__(self, inplanes, normalize=False):
        super(inorm_block_different, self).__init__()
        self.normalize = normalize
        #self.downsample = downsample
        #if self.downsample:
        #    self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
...
class block4(torch.nn.Module): #does not begin with instancenorming, affine instancenorm
    def __init__(self, inplanes, normalize=False):
...
class SELayer(torch.nn.Module):
    def __init__(self,channel,reduction=16):
        ...
        '''        
        self.lin1 = torch.nn.Linear(channel, channel // reduction, bias=False)
        self.relu1 = torch.nn.ReLU(inplace=True)
        #self.sigm1 = torch.nn.Tanh()
        self.lin2 = torch.nn.Linear(channel // reduction, channel, bias=False)
...
class se_block4(torch.nn.Module): #does not begin with instancenorming, affine instancenorm
    def __init__(self, inplanes, normalize=False):
        super(se_block4, self).__init__()
        self.normalize = normalize
        #self.downsample = downsample
        #if self.downsample:
        #    self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
...
class inorm_net(torch.nn.Module): #conv1,2,3,4 here are similar to densenet
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        ...
        self.conv1_i = torch.nn.Conv3d(self.inplanes, channels, kernel_size=7, stride=2, padding=2, bias=False)
        self.norm1_i = torch.nn.InstanceNorm3d(channels,affine=True)
        #self.relu1 = torch.nn.ReLU(inplace=True)
        self.inplanes = channels
        ...
        self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        #self.conv2_2 = torch.nn.Conv3d(self.inplanes, self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        #self.inplanes = 2 * self.inplanes
        self.norm2_i = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu2_i = torch.nn.ReLU(inplace=True)

        self.layer2 = self._make_layer(block, layers[1], normalize=True)

        self.maxpool2 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        #self.conv3 = torch.nn.Conv3d(self.inplanes, self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        #self.inplanes = 2 * self.inplanes
        self.norm3_i = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu3_i = torch.nn.ReLU(inplace=True)

        self.layer3 = self._make_layer(block, layers[2], normalize=True)

        self.maxpool3 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        #self.conv4 = torch.nn.Conv3d(self.inplanes, self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        #self.inplanes = 2 * self.inplanes
        self.norm4_i = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu4_i = torch.nn.ReLU(inplace=True)
        ...
        self.avgpool = torch.nn.AdaptiveAvgPool3d((1, 1, 1))
        #self.do = torch.nn.Dropout(p=dropoutrate)
        self.fc = torch.nn.Linear(self.inplanes, numoutputs)
        #self.tanh = torch.nn.Tanh()
...
class inorm_net_convnormrelu(torch.nn.Module): #conv1,2,3,4 here are similar to densenet
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        super(inorm_net_convnormrelu, self).__init__()
        ...
        #self.norm_final = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu_final = torch.nn.ReLU()
        self.avgpool = torch.nn.AdaptiveAvgPool3d((1, 1, 1))
        #self.do = torch.nn.Dropout(p=dropoutrate)
        self.fc = torch.nn.Linear(self.inplanes, numoutputs)
...
class inorm_net_doubling(torch.nn.Module): #conv1,2,3,4 here are similar to densenet
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        super(inorm_net_doubling, self).__init__()
        ...
        self.conv2_i = torch.nn.Conv3d(self.inplanes, 2 * self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        self.maxpool1 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        self.inplanes = 2 * self.inplanes
        self.norm2_i = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu2_i = torch.nn.ReLU(inplace=True)
        ...
        self.maxpool2 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        self.conv3_i = torch.nn.Conv3d(self.inplanes, 2 * self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        self.inplanes = 2 * self.inplanes
        self.norm3_i = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu3_i = torch.nn.ReLU(inplace=True)
        ...
        #self.maxpool3 = torch.nn.MaxPool3d(kernel_size=2, stride=2)
        self.conv4_i = torch.nn.Conv3d(self.inplanes, 2*self.inplanes, kernel_size=3, stride=2, padding=1, bias=False)
        self.inplanes = 2 * self.inplanes
        self.norm4_i = torch.nn.InstanceNorm3d(self.inplanes, affine=True)
        #self.relu4_i = torch.nn.ReLU(inplace=True)
...
class inorm_net_convnormrelu_4(torch.nn.Module): #conv1,2,3,4 here are similar to densenet
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        super(inorm_net_convnormrelu_4, self).__init__()
        ...
        for m in self.modules():
            if isinstance(m, inorm_block_convnormrelu_4):
                print("zero initializing norm4")
                torch.nn.init.constant_(m.norm4.weight, 0.01)
                torch.nn.init.constant_(m.norm4.bias, 0.01)
...
class inorm_net_different(torch.nn.Module): #conv1,2,3,4 here are similar to densenet
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        super(inorm_net_different, self).__init__()
...
class net_different(torch.nn.Module): #conv1,2,3,4 here are similar to densenet
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        super(net_different, self).__init__()
        ...
        self.layer1 = self._make_layer(block, layers[0], normalize=False)
        ...
        self.layer2 = self._make_layer(block, layers[1], normalize=False)
        ...
        self.layer3 = self._make_layer(block, layers[2], normalize=False)
        ...
        self.layer4 = self._make_layer(block, layers[3], normalize=False)
        #self.layer5 = self._make_layer(block, layers[4])
        ...
        for m in self.modules():
            ...
            #    torch.nn.init.kaiming_uniform_(m.weight, a=1.0)
                ...
                torch.nn.init.kaiming_uniform_(m.lin1.weight)
...
class NetConstant_noBN_l4_leaky(torch.nn.Module):
    def __init__(self, block, layers, numoutputs, channels, includesft=False):
        super(NetConstant_noBN_l4_leaky, self).__init__()
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv3d):
                torch.nn.init.kaiming_normal_(m.weight, a=0.3, mode='fan_out')
            elif isinstance(m, torch.nn.BatchNorm3d):
                #torch.nn.init.constant_(m.weight, 1)
                #torch.nn.init.constant_(m.bias, 0)
                raise Exception("no batchnorm")
...
def inormnet_new_light(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net(inorm_block, [4,4,4,4], numoutputs, 64, includesft=includesft)

def inormnet_new_light_convnormrelu(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net_convnormrelu(inorm_block_convnormrelu, [4,4,4,4], numoutputs, 64, includesft=includesft)

def inormnet_new_128(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net(inorm_block, [4,4,4,4], numoutputs, 128, includesft=includesft)

def inormnet_new_doubling(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net_doubling(inorm_block, [4,4,4,2], numoutputs, 32, includesft=includesft)

def inormnet_new_doubling_4(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net_doubling(inorm_block4, [2,2,2,1], numoutputs, 32, includesft=includesft)

def inormnet_new_light_convnormrelu_4(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net_convnormrelu_4(inorm_block_convnormrelu_4, [2,2,2,2], numoutputs, 64, includesft=includesft)

def inormnet_new_light_different(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net_different(inorm_block_different, [4,4,4,4], numoutputs, 128, includesft=includesft)

def inormnet_new_light4(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net(inorm_block4, [2,2,2,2], numoutputs, 64, includesft=includesft)

def inormnet_new_light_128(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net(inorm_block, [4,4,4,4], numoutputs, 128, includesft=includesft)

def inormnet_new_4(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return inorm_net(inorm_block4, [2,2,2,2], numoutputs, 128, includesft=includesft)

def new_4(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return net_different(block4, [2,2,2,2], numoutputs, 128, includesft=includesft)

def se_new_4(numoutputs,dropoutrate,includesft): #similar to netconstant_IN_norm, checking performance diff
    return net_different(se_block4, [2,2,2,2], numoutputs, 64, includesft=includesft)
