...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with np.load(file_path + "Data_0001_thr2.npz") as data:
            ...
            thrvolume = data['data']
            thrvolume_resized = np.delete(np.delete(np.delete(thrvolume, 128, 0), 128, 1), 128,
                                          2)  # from 129x129x129 to 128x128x128
        ...
        with open(file_path + "parameter_tag2.pkl", "rb") as par:
            ...
            alpha = np.sqrt(params['Dw'] / params['rho'])
            beta = params['Tend'] * params['rho']
            paramsarray[0] = np.interp(alpha, [np.sqrt(0.001), np.sqrt(7.5)], normalization_range)
            paramsarray[1] = np.interp(beta, [0.1, 300], normalization_range)
            ...
            assert paramsarray[0] != -1 and paramsarray[0] != 1
            assert paramsarray[1] != -1 and paramsarray[1] != 1
