...
#this version has some extra calc and removes safety check that you actually have evaluated all data that you wanted (so you can execute outside of giga)
...
for i in range(0, len(thresholds)):
    ...
    for result in allresults:
        assert result['diceresults'][i][0] == threshold
        result_infiltration = ((result['mu1_gt'] / result['mu2_gt']) * 10)**2 #D/p in mm^2

        if (result_infiltration >= 2) and (result_infiltration <= 20):
            dice = result['diceresults'][i][1]  # dice score for this threshold
            scores_for_threshold.append(dice)
    ...
    plt.title("DICE >= " + str(threshold) + ", mean = " + str(
        np.round(np.array(scores_for_threshold).mean(), 3)) + ", nans = " + str(nan_scores) + ", evaluated: " + str(
        scoreslen))
    plt.savefig('dice_' + str(threshold) + '_insideHGG.png')
    plt.close(f)


infiltr_array=[]
for result in allresults:
    assert result['diceresults'][11][0] == 0.9
    result_infiltration = ((result['mu1_gt'] / result['mu2_gt']) * 10) #sqrt(D/p) in mm
    if result['diceresults'][11][1] < 0.02:
        infiltr_array.append(result_infiltration)

f = plt.figure()
plt.hist(infiltr_array, bins=50, figure=f)
plt.title(f"mean: {np.mean(infiltr_array)}, evaluated: {len(infiltr_array)}")
plt.savefig('testset0dice_' + str(threshold) + '_075.png')
plt.close(f)
